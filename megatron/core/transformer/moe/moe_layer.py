...
class CoELayer(BaseMoELayer):
    ...
    def __init__(
        ...
        if self.config.coe_shared_router:
            ...
            self.routers = torch.nn.ModuleList([TopKRouter(config=self.config, model_comm_pgs=model_comm_pgs) 
                                                    for _ in range(self.coe_communication_steps)])
