...
@jit_fuser
def bias_swiglu_without_silu(y, bias):
    y = y + bias
    return swiglu_without_silu(y)
...
@jit_fuser
def bias_swiglu_without_silu_back(g, y, bias):
    y = y + bias
    return swiglu_without_silu_back(g, y)
...
class BiasSwiGLUWithoutSiluFunction(torch.autograd.Function):
    """Custom autograd function for SwiGLU activation with bias support."""
    ...
    def forward(ctx, input, bias, fp8_input_store, cpu_offload_input):
        ...
        ctx.ori_input_dtype = input.dtype
        ctx.fp8_input_store = fp8_input_store
        return bias_swiglu_without_silu(input, bias)
    ...
    def backward(ctx, grad_output):
        ...
        input, bias = ctx.saved_tensors
        input = input.to(ctx.ori_input_dtype) if ctx.fp8_input_store else input
        tmp = bias_swiglu_without_silu_back(grad_output, input, bias)
        return tmp, tmp, None, None
...
class SwiGLUWithoutSiluFunction(torch.autograd.Function):
    """Custom autograd function for SwiGLU activation without bias."""
    ...
    def forward(ctx, input, fp8_input_store, cpu_offload_input):
        ...
        ctx.ori_input_dtype = input.dtype
        ctx.fp8_input_store = fp8_input_store
        return swiglu_without_silu(input)
    ...
    def backward(ctx, grad_output):
        ...
        input = ctx.saved_tensors[0]
        input = input.to(ctx.ori_input_dtype) if ctx.fp8_input_store else input
        tmp = swiglu_without_silu_back(grad_output, input)
        return tmp, None, None
...
def bias_swiglu_without_silu_impl(input, bias, fp8_input_store=False, cpu_offload_input=False):
    """Implementation of biased SwiGLU that handles different input shapes.
    ...
    input = input.view(-1, ori_shape[-1])
    if bias is not None:
        output = BiasSwiGLUWithoutSiluFunction.apply(input, bias, fp8_input_store, cpu_offload_input)
    else:
        output = SwiGLUWithoutSiluFunction.apply(input, fp8_input_store, cpu_offload_input)
