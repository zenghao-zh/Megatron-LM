...
def generate_new_hadamard(H):
    n = H.shape[0]
    D = torch.randint(0, 2, (n,), dtype=H.dtype, device=H.device) * 2 - 1
    D = torch.diag(D)
    H_new = D @ H
    return H_new

def hadamard_rotate(x, rotate_h, dim=-1):
    if rotate_h is None:
        return x
    else:
        rotate_dim = rotate_h.shape[0]
        original_shape = x.shape
        x = x.reshape(-1, rotate_dim)@rotate_h
...
def hadamard_rotate(x, rotate_h, dim=-1):
    if rotate_h is None:
        return x
    x_transposed = torch.moveaxis(x, dim, -1)
    transposed_shape = x_transposed.shape
    rotate_dim_size = rotate_h.shape[0]
    x_reshaped = x_transposed.reshape(-1, rotate_dim_size)
    rotated_reshaped = x_reshaped @ rotate_h
    rotated_transposed = rotated_reshaped.reshape(transposed_shape)
    result = torch.moveaxis(rotated_transposed, -1, dim)
    return result
...
def matmul_partial_hadU_cuda(X, rotate_h, rotate_k):
    B, N, C = X.shape
    input = X.view(B, N, -1, rotate_k)
    input = input @ rotate_h
    return input.reshape(X.shape)
